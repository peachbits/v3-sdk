  ...
  public static async bestTradeExactIn<TInput extends Currency, TOutput extends Currency>(
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      try {
        ...
        // input too low
        if (error instanceof Error && Object.getOwnPropertyNames(error).includes('isInsufficientInputAmountError')) {
          continue
        }
        throw error
      }
  ...
  public static async bestTradeExactOut<TInput extends Currency, TOutput extends Currency>(
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      try {
        ...
        if (error instanceof Error && Object.getOwnPropertyNames(error).includes('isInsufficientReservesError')) {
          continue
        }
        throw error
      }
