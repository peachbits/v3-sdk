...
describe('Trade', () => {
    ...
    function v2StylePool(reserve0, reserve1, feeAmount = FeeAmount.MEDIUM) {
        ...
        return new Pool(reserve0.currency, reserve1.currency, feeAmount, sqrtRatioX96, liquidity, TickMath.getTickAtSqrtRatio(sqrtRatioX96), [
            {
    ...
    const pool_0_3 = v2StylePool(CurrencyAmount.fromRawAmount(token0, 100000), CurrencyAmount.fromRawAmount(token3, 90000));
    const pool_1_2 = v2StylePool(CurrencyAmount.fromRawAmount(token1, 120000), CurrencyAmount.fromRawAmount(token2, 100000));
    const pool_1_3 = v2StylePool(CurrencyAmount.fromRawAmount(token1, 120000), CurrencyAmount.fromRawAmount(token3, 130000));
    const pool_weth_0 = v2StylePool(CurrencyAmount.fromRawAmount(WETH9[1], JSBI.BigInt(100000)), CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100000)));
    const pool_weth_1 = v2StylePool(CurrencyAmount.fromRawAmount(WETH9[1], JSBI.BigInt(100000)), CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(100000)));
    const pool_weth_2 = v2StylePool(CurrencyAmount.fromRawAmount(WETH9[1], JSBI.BigInt(100000)), CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100000)));
    describe('#fromRoute', () => {
        it('can be constructed with ETHER as input', async () => {
            const trade = await Trade.fromRoute(new Route([pool_weth_0], ETHER, token0), CurrencyAmount.fromRawAmount(Ether.onChain(1), JSBI.BigInt(10000)), TradeType.EXACT_INPUT);
            expect(trade.inputAmount.currency).toEqual(ETHER);
            expect(trade.outputAmount.currency).toEqual(token0);
        });
        it('can be constructed with ETHER as input for exact output', async () => {
            const trade = await Trade.fromRoute(new Route([pool_weth_0], ETHER, token0), CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10000)), TradeType.EXACT_OUTPUT);
            expect(trade.inputAmount.currency).toEqual(ETHER);
            expect(trade.outputAmount.currency).toEqual(token0);
        });
        it('can be constructed with ETHER as output', async () => {
            const trade = await Trade.fromRoute(new Route([pool_weth_0], token0, ETHER), CurrencyAmount.fromRawAmount(Ether.onChain(1), JSBI.BigInt(10000)), TradeType.EXACT_OUTPUT);
            expect(trade.inputAmount.currency).toEqual(token0);
            expect(trade.outputAmount.currency).toEqual(ETHER);
        });
        it('can be constructed with ETHER as output for exact input', async () => {
            const trade = await Trade.fromRoute(new Route([pool_weth_0], token0, ETHER), CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10000)), TradeType.EXACT_INPUT);
            expect(trade.inputAmount.currency).toEqual(token0);
            expect(trade.outputAmount.currency).toEqual(ETHER);
        });
    });
    ...
    describe('#createUncheckedTrade', () => {
        it('throws if input currency does not match route', () => {
            expect(() => Trade.createUncheckedTrade({
    ...
    describe('#createUncheckedTradeWithMultipleRoutes', () => {
        it('throws if input currency does not match route with multiple routes', () => {
            expect(() => Trade.createUncheckedTradeWithMultipleRoutes({
                routes: [
                    {
    ...
    describe('#bestTradeExactIn', () => {
        it('throws with empty pools', async () => {
            await expect(Trade.bestTradeExactIn([], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10000)), token2)).rejects.toThrow('POOLS');
        });
        it('throws with max hops of 0', async () => {
            await expect(Trade.bestTradeExactIn([pool_0_2], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10000)), token2, {
                maxHops: 0
            })).rejects.toThrow('MAX_HOPS');
        });
        it('provides best route', async () => {
            const result = await Trade.bestTradeExactIn([pool_0_1, pool_0_2, pool_1_2], CurrencyAmount.fromRawAmount(token0, 10000), token2);
        ...
        it('respects maxHops', async () => {
            const result = await Trade.bestTradeExactIn([pool_0_1, pool_0_2, pool_1_2], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10)), token2, { maxHops: 1 });
            expect(result).toHaveLength(1);
            expect(result[0].swaps[0].route.pools).toHaveLength(1); // 0 -> 2 at 10:11
            expect(result[0].swaps[0].route.tokenPath).toEqual([token0, token2]);
        });
        it('insufficient input for one pool', async () => {
            const result = await Trade.bestTradeExactIn([pool_0_1, pool_0_2, pool_1_2], CurrencyAmount.fromRawAmount(token0, 1), token2);
            expect(result).toHaveLength(2);
            expect(result[0].swaps[0].route.pools).toHaveLength(1); // 0 -> 2 at 10:11
            expect(result[0].swaps[0].route.tokenPath).toEqual([token0, token2]);
        ...
        it('respects n', async () => {
            const result = await Trade.bestTradeExactIn([pool_0_1, pool_0_2, pool_1_2], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10)), token2, { maxNumResults: 1 });
            expect(result).toHaveLength(1);
        });
        it('no path', async () => {
            const result = await Trade.bestTradeExactIn([pool_0_1, pool_0_3, pool_1_3], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10)), token2);
            expect(result).toHaveLength(0);
        });
        it('works for ETHER currency input', async () => {
            const result = await Trade.bestTradeExactIn([pool_weth_0, pool_0_1, pool_0_3, pool_1_3], CurrencyAmount.fromRawAmount(Ether.onChain(1), JSBI.BigInt(100)), token3);
            expect(result).toHaveLength(2);
        ...
        it('works for ETHER currency output', async () => {
            const result = await Trade.bestTradeExactIn([pool_weth_0, pool_0_1, pool_0_3, pool_1_3], CurrencyAmount.fromRawAmount(token3, JSBI.BigInt(100)), ETHER);
            expect(result).toHaveLength(2);
    ...
    describe('#maximumAmountIn', () => {
        describe('tradeType = EXACT_INPUT', () => {
            let exactIn;
            beforeEach(async () => {
                exactIn = await Trade.fromRoute(new Route([pool_0_1, pool_1_2], token0, token2), CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), TradeType.EXACT_INPUT);
            });
            it('throws if less than 0', () => {
        ...
        describe('tradeType = EXACT_OUTPUT', () => {
            let exactOut;
            beforeEach(async () => {
                exactOut = await Trade.fromRoute(new Route([pool_0_1, pool_1_2], token0, token2), CurrencyAmount.fromRawAmount(token2, 10000), TradeType.EXACT_OUTPUT);
            });
            it('throws if less than 0', () => {
            ...
            it('returns slippage amount if nonzero', () => {
                expect(exactOut
                    ...
                    .equalTo(CurrencyAmount.fromRawAmount(token0, 15488))).toBeTruthy();
                expect(exactOut
                    .maximumAmountIn(new Percent(JSBI.BigInt(5), JSBI.BigInt(100)))
                    .equalTo(CurrencyAmount.fromRawAmount(token0, 16262))).toBeTruthy();
                expect(exactOut
                    .maximumAmountIn(new Percent(JSBI.BigInt(200), JSBI.BigInt(100)))
                    .equalTo(CurrencyAmount.fromRawAmount(token0, 46464))).toBeTruthy();
            });
        });
    });
    describe('#minimumAmountOut', () => {
        describe('tradeType = EXACT_INPUT', () => {
            let exactIn;
            beforeEach(async () => (exactIn = await Trade.fromRoute(new Route([pool_0_1, pool_1_2], token0, token2), CurrencyAmount.fromRawAmount(token0, 10000), TradeType.EXACT_INPUT)));
            it('throws if less than 0', () => {
        ...
        describe('tradeType = EXACT_OUTPUT', () => {
            let exactOut;
            beforeEach(async () => {
                exactOut = await Trade.fromRoute(new Route([pool_0_1, pool_1_2], token0, token2), CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100)), TradeType.EXACT_OUTPUT);
            });
            it('throws if less than 0', () => {
            ...
            it('returns slippage amount if nonzero', () => {
                expect(exactOut
                    ...
                    .equalTo(CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100)))).toBeTruthy();
                expect(exactOut
    ...
    describe('#bestTradeExactOut', () => {
        it('throws with empty pools', async () => {
            await expect(Trade.bestTradeExactOut([], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100)))).rejects.toThrow('POOLS');
        });
        it('throws with max hops of 0', async () => {
            await expect(Trade.bestTradeExactOut([pool_0_2], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100)), {
                maxHops: 0
            })).rejects.toThrow('MAX_HOPS');
        });
        it('provides best route', async () => {
            const result = await Trade.bestTradeExactOut([pool_0_1, pool_0_2, pool_1_2], token0, CurrencyAmount.fromRawAmount(token2, 10000));
            expect(result).toHaveLength(2);
            expect(result[0].swaps[0].route.pools).toHaveLength(1); // 0 -> 2 at 10:11
            expect(result[0].swaps[0].route.tokenPath).toEqual([token0, token2]);
        ...
        it('respects maxHops', async () => {
            const result = await Trade.bestTradeExactOut([pool_0_1, pool_0_2, pool_1_2], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(10)), { maxHops: 1 });
            expect(result).toHaveLength(1);
            expect(result[0].swaps[0].route.pools).toHaveLength(1); // 0 -> 2 at 10:11
            expect(result[0].swaps[0].route.tokenPath).toEqual([token0, token2]);
        });
        it.skip('insufficient liquidity', () => {
            const result = Trade.bestTradeExactOut([pool_0_1, pool_0_2, pool_1_2], token0, CurrencyAmount.fromRawAmount(token2, 1200));
            expect(result).toHaveLength(0);
        });
        it.skip('insufficient liquidity in one pool but not the other', () => {
            const result = Trade.bestTradeExactOut([pool_0_1, pool_0_2, pool_1_2], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(1050)));
            expect(result).toHaveLength(1);
        });
        it('respects n', async () => {
            const result = await Trade.bestTradeExactOut([pool_0_1, pool_0_2, pool_1_2], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(10)), { maxNumResults: 1 });
            expect(result).toHaveLength(1);
        });
        it('no path', async () => {
            const result = await Trade.bestTradeExactOut([pool_0_1, pool_0_3, pool_1_3], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(10)));
            expect(result).toHaveLength(0);
        });
        it('works for ETHER currency input', async () => {
            const result = await Trade.bestTradeExactOut([pool_weth_0, pool_0_1, pool_0_3, pool_1_3], ETHER, CurrencyAmount.fromRawAmount(token3, 10000));
            expect(result).toHaveLength(2);
            expect(result[0].inputAmount.currency).toEqual(ETHER);
            expect(result[0].swaps[0].route.tokenPath).toEqual([WETH9[1], token0, token1, token3]);
            expect(result[0].outputAmount.currency).toEqual(token3);
        ...
        it('works for ETHER currency output', async () => {
            const result = await Trade.bestTradeExactOut([pool_weth_0, pool_0_1, pool_0_3, pool_1_3], token3, CurrencyAmount.fromRawAmount(Ether.onChain(1), JSBI.BigInt(100)));
            expect(result).toHaveLength(2);
            expect(result[0].inputAmount.currency).toEqual(token3);
            expect(result[0].swaps[0].route.tokenPath).toEqual([token3, token0, WETH9[1]]);
            expect(result[0].outputAmount.currency).toEqual(ETHER);
